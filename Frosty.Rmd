...
df <- df[1:10,]
print(df)
ggplot(df, aes(x = reorder(depto_ocu, count), y = count)) + geom_bar(fill = "blue", stat = "identity") + geom_text(aes(label = count), vjust = -0.5) + theme_pubclean()
```
...
names(datosInsi)[1] <- "anio"
names(datosInsi)[2] <- "mes"	
names(datosInsi)[3] <- "dia"
