...
```{r}
#Convert categorical dataset into numerical categorical
datosSAT<-read.csv("importacionesVehiculosSAT.csv", stringsAsFactors = FALSE)

datosSAT$Pais.de.Proveniencia <- as.numeric(factor(datosSAT$Pais.de.Proveniencia))
datosSAT$Aduana.de.Ingreso <- as.numeric(factor(datosSAT$Aduana.de.Ingreso))
datosSAT$Fecha.de.la.Poliza <- as.numeric(factor(datosSAT$Fecha.de.la.Poliza))
...
datosSAT$Tipo.de.Importador <- as.numeric(factor(datosSAT$Tipo.de.Importador))
datosSAT$Tipo.Combustible <- as.numeric(factor(datosSAT$Tipo.Combustible))
datosSAT
#>>>>>>> 6a7ba32c8373f72a05f2ebf489ac53650af34e8a
```
