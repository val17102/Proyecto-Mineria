...
```{r}
library(naivebayes)
modeloNb<-naive_bayes(as.factor(train$estado_implicado)~.,usekernel=T,data=train)
...
library(JOUSBoost)
#model_adaboost <- adaboost(train[,c(-11)], train[,11], tree_depth = 3, n_rounds = 100, verbose = FALSE,
  control = NULL)
```
