...
```{r}
unos <-dataMezclada[which(dataMezclada$estado_implicado == 1), ]
nrow(unos)
```

```{r}
balanceado <- rbind(dataMezclada, unos)
...
```{r}
library(adabag)
modelo <- boosting(train$estado_implicado~.,data=train)
prediccionRF1<-predict(modeloRF1,newdata = test)
```
...
```{r}
library(e1071)
modeloNb<-naiveBayes(train$estado_implicado~.,data=train)
predBayes<-predict(modeloNb, newdata = test[,-11])
predBayes
confusionMatrix(table(test$estado_implicado,predBayes))
```
...
```{r}
modeloRL<-glm(uno~., data = dataMezclada2[,c(1:16)],family = binomial(link='logit'), maxit=10000)
